...
public class DNG_MapModuleOffline : MonoBehaviour
{
    //[System.Serializable]
    ...
    //    if (followPlayer) ToggleFollowPlayer();
    //    if (layerChunks.Keys.Count == 0) return;
    //    if (currentLayer <= _minLayer) return;
    //    SetRenderLayer(currentLayer - 1);
    //}
    ...
    //    ColorUtility.TryParseHtmlString(followPlayer ? "#DC9632" : "#4D4D4D", out Color target);
    ...
    //    if (layerChunks.Keys.Count > 0)
    //    {
    //        _minLayer = layerChunks.Keys.Min();
    //        _maxLayer = layerChunks.Keys.Max();
    //    }
